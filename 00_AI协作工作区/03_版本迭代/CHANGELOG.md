...
## fix-project-save-and-timeline-axis v1.0 - 2026-06-20

- 修复 CloudBase 项目信息保存的写后回读误报：项目保存后使用有限次数回读确认，覆盖 CloudBase 写入短暂传播延迟场景；错误体、回读为空和最终业务字段不一致仍会失败。
- 增强 CloudBase 权限诊断：当 `update` 返回 `updated: 0` 时，直接提示“没有记录被更新，请检查集合写权限或文档归属”，避免被误判为普通回读不一致。
- 调整后台维护页按钮位置：保存项目信息位于项目信息框左下角；保存任务信息位于任务信息框左下角，并排在归档/恢复按钮之前。
...
## dashboard-and-timeline v1.1 - 2026-06-21

- 仪表盘新增"未启动"指标卡片，统计所有未设置实际开始日期的未归档任务。
- 后台维护表单交换里程碑与任务 ID 输入顺序，先输入里程碑再输入任务 ID。
- 时间轴改为双条形对比：计划周期用蓝色宽条，实际周期用红色窄条居中重叠，无实际日期仅显示蓝条。
- 导航栏移除"后台维护"入口链接，改为手动输入 `/admin` 访问。
- 资源方和责任人改为非必填字段，同步移除前端验证与 CloudBase 文档字段检查。
- 项目计划周期根据任务日期自动扩展；后台编辑项目日期不得窄于任务日期范围。
- 修复 `projectService.ts` 的 `hasRequiredTaskFields` 仍要求资源方/责任人非空，导致新增不填字段任务后仪表盘回退种子数据的问题（Claude Code 终审发现）。
- 验证：`npx vitest run` 通过，14 个测试文件、91 条测试用例通过。
- 验证：`npm run build` 通过。
- 可交给 Claude Code 审查：是。

## task-duration-and-status v1.2 - 2026-06-21

- 任务明细表在"计划周期"列右侧新增"计划工期"列，显示计划结束与计划开始的自然日天数。
- 任务明细表在"实际周期"列右侧新增"实际工期"列：有实际开始和结束显示天数，仅有实际开始显示"进行中"，无实际开始显示"-"。
- 状态列从单标签改为多标签组合，可同时展示启动偏差与结束偏差，用顿号分隔。
- 启动偏差：实际开始晚于计划开始显示"延迟启动"，早于显示"提前启动"，相等不显示。
- 已完成结束偏差：实际结束晚于计划结束显示"超期X天"，早于显示"提前X天"，相等不显示；无任何偏差时显示"已完成"。
- 进行中任务保留基于今天的实时预警：今日到期、7日内到期、延期X天。
- 未开始任务括号显示距计划结束天数"（距X天）"，已过计划结束日显示"（已超期X天）"。
- 多标签着色：绿色（提前启动/提前X天）、黄色（延迟启动/临期预警）、红橙色（超期X天/延期X天），中性色（无偏差）。
- `riskLabel: string` 改为 `riskLabels: string[]`，RiskTaskStrip 同步适配数组渲染与着色。
- 已完成但超期的任务在状态列显示"超期X天"但不进入风险任务条（维持 V1.1 行为）。
